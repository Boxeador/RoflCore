...
/* ScriptData
Author: Trinitycore ,edited by MadMan
            ...
            if (!UpdateVictim())
                ...
                switch(uiPhase)
                {
                        case PHASE_UNDEAD:
                    {
                        ...
                        {
                            DoCast(me->getVictim(), RAID_MODE(SPELL_PLAGUE_STRIKE, SPELL_PLAGUE_STRIKE_2));
                            uiPlagueStrikeTimer = urand(12000,15000);
                        } else uiPlagueStrikeTimer -= uiDiff;

                        if (uiObliterateTimer <= uiDiff)
                        {
                            DoCast(me->getVictim(), RAID_MODE(SPELL_OBLITERATE, SPELL_OBLITERATE_H));
                            uiObliterateTimer = urand(17000,19000);
                    ...
                    case PHASE_GHOST:
                    {
                            if (uiDeathBiteTimer <= uiDiff)
                        {
                            ...
                            DoCast(me, RAID_MODE(SPELL_DEATH_BITE, SPELL_DEATH_BITE_H));
                            uiDeathBiteTimer = urand (2000, 4000);
                        ...
                        if (uiMarkedDeathTimer <= uiDiff)
                        {
                            ...
                            {
                                if (pTarget && pTarget->isAlive())
                                DoCast(me->getVictim(), RAID_MODE(SPELL_MARKED_DEATH, SPELL_MARKED_DEATH_2));
                            }
                                uiMarkedDeathTimer = urand (5000, 7000);
        ...
        void WaypointReached(uint32 uiPointId)
        {
            switch(uiPointId)
            {
                    ...
                    me->SummonCreature(VEHICLE_GR,744.841f,634.505f,411.575f, 2.79f);
		              me->SummonCreature(NPC_BLACK_KNIGHT,744.841f,634.505f,411.575f, 2.79f);
                    break;
            }
        }
